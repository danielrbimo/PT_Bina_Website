...
public class Strategy {
     ...
     * through out the board on the given game state. The search goes from the left to right in
     * each row and moves down to the next row when one is filled up.
     *
    ...
    private AbstractMap.SimpleEntry<FishTreeNode, Integer> findMinimaxGainNode(FishTreeNode currentTreeNode, int nTurn, PenguinColor maximizingPlayerColor) throws IllegalArgumentException {
        ...
        } else if (nTurn == 0 || currentTreeNode.generateDirectReachableStates().isEmpty()) {
