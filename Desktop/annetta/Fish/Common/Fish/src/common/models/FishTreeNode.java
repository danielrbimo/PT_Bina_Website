 ...
 *
 * A node can be described in three kinds of states:
 * if a game is over, then there will be no child nodes for given parent node.
 * if a player is stuck, then the penguin positions in this node will still be in their same position
 * as the ones in the one child node's game state.
 * if a current player can move, then there will be more than one child nodes in which the penguin
 * placement are not the same.
 */
...
public class FishTreeNode {
   ...
   */
  public ArrayList<FishState> generateDirectReachableStates() {
    ArrayList<FishState> directReachableStates = new ArrayList<>();
...
//          FishTreeNode newTree = new FishTreeNode(nextState);
  ...
  public void generateChildNodes() {
    if (childNodes == null) {
      childNodes = new ArrayList<FishTreeNode>();
      for (FishState fishState : generateDirectReachableStates()) {
  ...
  /** A method that applies a action to the current state of a FishTreeNode.
   ...
   * @throws IllegalArgumentException if the action is an illegal move on the fishTreeNode.
   **/
  ...
  public ArrayList<FishState> applyActionToStates(FishTreeNode fishTreeNode, IAction action)
    ...
    ArrayList<FishState> states = fishTreeNode.generateDirectReachableStates();
