...
public class FishModel {
  ...
  public FishModel(int width, int height, int minOneFishNumOrFishNumOnTiles, boolean isRandomized) {
    ...
    if (width < 0 || height < 0) {
      throw new IllegalArgumentException("Error: cannot have negative width and height");
    }
    if (minOneFishNumOrFishNumOnTiles < 1 || minOneFishNumOrFishNumOnTiles > 5) {
      throw new IllegalArgumentException("Error: fish number can't be less than 1 or "
          + "greater than 5");
    }
